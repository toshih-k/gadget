    ...
    class DeleteMutationGenerator < Rails::Generators::NamedBase
      ...
      def modify_mutation
        inject_into_file 'app/graphql/types/mutation_type.rb',
                         "    field :delete_#{file_name}, mutation: Mutations::Delete#{name}Mutation\n",
                         before: "  end\nend\n"
      end

      def create_mutation_file
        template('mutations/delete.rb.tt', "app/graphql/mutations/delete_#{file_name}_mutation.rb", { name: name })
      end
    end
  end
end
